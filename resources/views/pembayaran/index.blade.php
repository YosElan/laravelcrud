    ...
    <div class="card shadow mb-4">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="m-1 btn btn-sm btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"><i class="fas fa-trash"></i></button><a href="#" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#editModal{{$row->id}}"><!-- Solid Style (fas) -->
        ...
        <div class="card-header py-3 ">
            <div class="float-right">
                <h6 class="m-1 font-weight-bold text-success">Ingin menyimpannya ke Excel?
                    <a class="m-1 btn btn-sm btn-success" id="exportButton" href="{{url('excel-export')}}"><i class="m-2 fas fa-file-excel"></i>Export</a>
                </h6>
            </div>
        </div>
    </div>
...
<script>
    ...
    Toast.fire({
        icon: 'success',
        title: 'Data Barang berhasil ditambah' // Perbaiki kesalahan penulisan "tittle" menjadi "title"
    })
    @endif
</script>
<script>
    ...
    function showAlert() {
        alert('Data akan diekspor ke Excel.');
        window.location.href = document.getElementById('exportButton').getAttribute('href');
    }
</script>
@endsection
